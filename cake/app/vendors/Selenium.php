...
class Testing_Selenium
{
    ...
    public function _findport()
    {
	$portAttempt = 4444;

	while(1):
	   while($portAttempt < 4447):
	      $checkport = fsockopen('localhost', $portAttempt, $errnum, $errstr, 300);
	   
	      if(!$checkport) {
		switch ($portAttempt) {
		  case 4444:
			echo "start selenium server on port 4444 ";
			$command = "/bin/bash /data/laboratree/selenium/start.sh 4444";
        		exec($command);
			return 4444;
		  case 4445:
			echo "start selenium server on port 4445 ";
			$command = "/data/laboratree/selenium/start-selenium.sh 4445";
                        exec($command);
		sleep(3);
				while(1):
					$checkport = fsockopen('localhost', 4445, $errnum, $errstr, 300);
      					if($checkport) {
						return 4445;
					}
				endwhile;
		  case 4446:
			echo "start selenium server on port 4446 ";
			$command = "/bin/bash /data/laboratree/selenium/start-selenium.sh 4446";
 //                       exec($command);
			return 4446;
		}
	      }
     ...
     */
    public function __construct($browser, $browserUrl, $host = 'localhost', $port = 4444, $timeout = 10000)
    {
...
//	$command = "/bin/bash /data/laboratree/selenium/start-selenium.sh " . $this->port;
//	exec($command);
//sleep(30);
 ...
 //   {
	// Remove Port Files
//	unlink("/tmp/port." . $this->port);

//	$command = "/bin/bash /data/laboratree/selenium/stop-test-server.sh " . $this->port;
//	exec($command);
//    }
