<?php 
/* SVN FILE: $Id$ */
/* Preference Test cases generated on: 2010-12-20 14:58:57 : 1292857137*/
...
class PreferenceTestCase extends CakeTestCase {
	...
	function testPreferenceInstance() {
		$this->assertTrue(is_a($this->Preference, 'Preference'));
	}

	function testPreferenceFind() {
		$this->Preference->recursive = -1;
		$results = $this->Preference->find('first');
		$this->assertTrue(!empty($results));

		$expected = array('Preference' => array(
			'id'  => 1,
	...
	function testToNode() {
		$this->Preference->recursive = -1;
		$results = $this->Preference->find('first');
		$node = $this->Preference->toNode($results);
	...
	function testToNodeNull() {
		try
		{
			$node = $this->Preference->toNode(null);
	...
	function testToNodeNotArray() {
		try
		{
			$node = $this->Preference->toNode('string');
			$this->fail('InvalidArgumentException was expected');
		}
		catch (InvalidArgumentException $e)
		{
...
//Removed Because ToNode does not test these any more
/*
	function testToNodeMissingModel() {
		try
		{
			$node = $this->Preference->toNode(array('id' => 1));
			$this->fail('InvalidArgumentException was expected');
		}
		catch (InvalidArgumentException $e)
		{
	...
	function testToNodeMissingKey() {
		try
		{
			$node = $this->Preference->toNode(array('Preference' => array('test' => 1)));
			$this->fail('InvalidArgumentException was expected');
		}
		catch (InvalidArgumentException $e)
		{
