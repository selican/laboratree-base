<?php 
/* SVN FILE: $Id$ */
/* UsersPreference Test cases generated on: 2010-12-20 15:01:56 : 1292857316*/
App::import('Model', 'UsersPreference');

class UsersPreferenceTestCase extends CakeTestCase {
	var $UsersPreference = null;
	...
	function startTest() {
		$this->UsersPreference =& ClassRegistry::init('UsersPreference');
	}

	function testUsersPreferenceInstance() {
		$this->assertTrue(is_a($this->UsersPreference, 'UsersPreference'));
	}

	function testUsersPreferenceFind() {
		$this->UsersPreference->recursive = -1;
		$results = $this->UsersPreference->find('first');
		$this->assertTrue(!empty($results));

		$expected = array('UsersPreference' => array(
			'id'  => 1,
			'table_type' => 'user',
			'table_id'  => 1,
			'user_id'  => 1,
