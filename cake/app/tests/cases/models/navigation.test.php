...
class NavigationTestCase extends CakeTestCase {
        ...
        function testToNodeNull() {
                try
                {
                        $node = $this->Navigation->toNode(null);
                }
	...
	function testSortTabsNull() {
		try
		{
			$this->Navigation->_sortTabs();
		}
		 catch (Exception $e)
                {
                        $this->pass();
	...
	function testProcessEmpty() {
        ...
        function testProcessNull() {
                try
                {
                        $this->Navigation->_process(null, null);
                }
                 catch (Exception $e)
                {
                        $this->pass();
        ...
        function testProcessStuff() {
                try
                {
                      //  $this->Navigation->_process('Perm', 'Stuff');
                }
                 catch (Exception $e)
                {
                        $this->fail();
	...
	function testProcessArrayStuff() {
		try
		{
//		$this->Navigation->_process('Perm', array('stuff', 'stuff'));
	
		//	$this->Navigation->_process('Perm', array('stuff', 'stuff'), array(array(1)));
		}
		catch(Exception $e)
		{
			$this->fail();
