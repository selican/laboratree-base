<?php 
/* SVN FILE: $Id$ */
/* Setting Test cases generated on: 2010-12-20 15:00:09 : 1292857209*/
...
class SettingTestCase extends CakeTestCase {
	...
	function testToNode() {
		$this->Setting->recursive = -1;
		$results = $this->Setting->find('first');
		$node = $this->Setting->toNode($results);

		$expected = array(
			'setting_id'  => 1,
			'setting_name' => 'Settings Root',
	...
	function testToNodeNotArray() {
		try
		{
			$node = $this->Setting->toNode('string');
			$this->fail('InvalidArgumentException was expected');
		}
		catch (InvalidArgumentException $e)
		{
...
//Removed because toNode does not test these anymore
/*
	function testToNodeMissingModel() {
		try
		{
			$node = $this->Setting->toNode(array('id' => 1));
			$this->fail('InvalidArgumentException was expected');
		}
		catch (InvalidArgumentException $e)
		{
	...
	function testToNodeMissingKey() {
		try
		{
			$node = $this->Setting->toNode(array('Setting' => array('test' => 1)));
			$this->fail('InvalidArgumentException was expected');
		}
		catch (InvalidArgumentException $e)
		{
